...
from custom_components.ha_solcast_fusion.const import (
    ...
    CONF_DC_W, CONF_AC_W, CONF_SOLCAST_KEY, CONF_SOLCAST_SITE,
)
...
SITE_SAME = {
    "resource_id": "rid", "name": "Home",
    "latitude": 52.0, "longitude": 4.0,
    "capacity": 5.0, "capacity_dc": 5.0, "azimuth": -180, "tilt": 30,
}
SITE_CHANGED = {**SITE_SAME, "tilt": 45}   # declination 30 -> 45

BASE_DATA = {
    CONF_SOLCAST_KEY: "k", CONF_SOLCAST_SITE: "rid",
    CONF_LAT: 52.0, CONF_LON: 4.0, CONF_DECLINATION: 30,
    CONF_AZIMUTH: 180, CONF_DC_W: 5000, CONF_AC_W: 5000,
}
...
async def test_changed_geometry_updates_and_refreshes():
    ...
    with patch("custom_components.ha_solcast_fusion.mirror.fetch_sites",
               new=AsyncMock(return_value=[SITE_CHANGED])):
...
async def test_unchanged_geometry_no_update():
    ...
    config = dict(BASE_DATA)
    with patch("custom_components.ha_solcast_fusion.mirror.fetch_sites",
               new=AsyncMock(return_value=[SITE_SAME])):
        await async_mirror_check(hass, entry, config, store, MagicMock(), coord)
...
async def test_already_synced_today_skips_fetch():
    ...
    fetch = AsyncMock()
    with patch("custom_components.ha_solcast_fusion.mirror._utc_today",
...
async def test_busy_error_does_not_mark_synced():
    ...
    store = _fake_store()
    with patch("custom_components.ha_solcast_fusion.mirror.fetch_sites",
               new=AsyncMock(side_effect=SolcastBusyError("429"))):
        await async_mirror_check(hass, entry, dict(BASE_DATA), store, MagicMock(), coord)
    store.mark_mirror_synced.assert_not_awaited()
    hass.config_entries.async_update_entry.assert_not_called()
...
async def test_dropped_capacity_removes_key_and_converges():
    ...
    with patch("custom_components.ha_solcast_fusion.mirror.fetch_sites",
               new=AsyncMock(return_value=[site_no_ac])):
        await async_mirror_check(hass, entry, config, store, MagicMock(), coord)
